...
const MUTATIONS = [
  ...
  // app.js showAppView (line 482) — Curie verified app.js:482's removal is redundant on the
  // abort reveal path and a mutant registered there would read UNCAUGHT
  // (Claude/Curie/RED-swipe-stage6g.md §3). Forcing setView to always park #home breaks the
  // reveal's un-park.
  { name: 'stage6g REVEAL: setView stops un-parking #home on a reveal (-> REVEAL test)',
    file: 'js/nav.js',
    from: "      $('home').classList.toggle('parked', v !== 'home');   // parked = off-screen but PAINTED (covers stay decoded)",
    to:   "      $('home').classList.toggle('parked', true);   // parked = off-screen but PAINTED (covers stay decoded)" },
];
