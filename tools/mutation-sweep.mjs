...
const SOURCE_TEXT_GATES = {
  ...
  'transition-matrix.test.js': 'REDERIVES its registry from source text on every run, by two channels: it regexes js/nav.js SETTINGS_SUBS, and it `require`s js/swipe.js at generator load (tools/gen-transition-matrix.mjs:34) for BROWSE_FAMILY — so any mutant targeting either file can fail it for a reason unrelated to its own subject (the derived inventory), the same false-CAUGHT class as the other entries here. (The js/app.js branch-region FINGERPRINT this reason used to name was retired at stage 4 — js/swipe.js now owns that decision and the frozen test/fixtures/swipe-plan-spec.mjs checks it structurally — so it is no longer a channel this gate can fail through.)',
