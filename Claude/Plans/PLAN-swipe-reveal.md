...
7. STAGED EXTRACTION
   ...
   7. Replace Browse hold calls with the lease interface. Detail:
      ...
      DEFERRED-to-7 clause against current source: SEVEN of its ten items
      have no subject in the tree (the de-clone removed every owned pane;
      two more had their premise inverted), so stage 7 is the Browse hold
      boundary and nothing else. Three items are re-deferred there with
      their consumers named. §3.5's `ready: Promise` and `settledScrollY`
      are REFUSED as dead fields; §14 of the subtraction plan's
      mover-lifetime invariant is OWED and lands with this stage.
