...
7. STAGED EXTRACTION
   ...
   6. Centralize finalization and reveal ordering (I10, I17). [SLICED: 6a supersession recovery done; 6b async-handle release-half done (settle/reveal timers session-owned + retired); 6c pane-LESS supersession + settle-phase cur===session identity guard done (overlay-involving set only); 6d the FIRST declarative finalization decision done — `finalizationPlanFor(classification).abortRender` (`'rerender'` iff browse→browse), a pure deep-frozen arm-time field on the session (`cur.finPlan`), retiring the runtime `sourceWasClobbered`/`d.clobbered` byproduct; the frozen `expectedFinalization` oracle turned on (behaviour-preserving extraction, no known-red — see PLAN-swipe-stage6d.md). 6e the pane-lifecycle interface's F(dispose) half done — the owner-driven, session-owned `disposeOwnedPanes(session, reason)` (js/app.js) replaces the DOM-global `.nav-ghost` sweep's owned-pane-removal effect at the `begin()`-recovery site for a pane-owning DRAGGING supersession (`reason:'superseded'`), threading `keepGhosts:true` at both the explicit `resetSwipeStyles` call and the `applyScreen` opts so the sweep does not duplicate the removal; the ORPHAN branch keeps the full sweep unchanged; behaviour-preserving extraction, no known-red — see PLAN-swipe-stage6e.md. 6f OPENED the STRUCTURAL-FIX axis (never transform the real in-flow view) with its first slice — the in-flow→overlay OUTGOING is now an owned-pane app-ghost (the shipped `ghostApp` recipe) instead of the transformed real in-flow view, so the real `#browse`/`#home` is never a mover and never receives a swipe transform on browse→overlay/home→overlay (and their NP-decorated members); a one-line `constructionPlanFor` decision-value flip (js/swipe.js), js/app.js UNTOUCHED, construction-REPRESENTATION NEW POLICY (EC §4.19), no known-red, guarded by the frozen `expectedConstruction` spec + the `swipe-transition` oracle; the compositor flash is device-only and NOT fixed here, and the no-peek for the inset overlay destinations is device-verified only — see PLAN-swipe-stage6f.md. 6g fixed the home→books ABORT flash on the reveal-side complement of "never demote the real view": `#home` is now a PERMANENT compositing layer via an unconditional stylesheet `#home { transform: translateZ(0) }` (css/app.css), so removing `.parked` at a reveal cannot demote it (the demote WAS the abort flash) — a scoped exception to the no-promotion invariant for `#home` ONLY (NEW POLICY, EC §4.19; `#browse` stays un-promoted), `translateZ(0)` over the droppable `will-change`, permanent over reveal-scoped, js/app.js COMMENT-ONLY, pinned by the source-text gate `test/home-layer-invariant.test.js` and the new general `caughtBy`/`gateTestsFor` source-text-mutation verification path in tools/mutation-sweep.mjs; DEVICE-confirmed for the `.256` `will-change` probe form, the shipped `translateZ(0)` form's device pass (abort flash clean, no navbar pop, active-home text quality) still OWED — see PLAN-swipe-stage6g.md. 6h gated the commit→home reveal cover-drop on a real scroll-settle signal — the DEVICE-PENDING fix for the commit books→home flash (flash A), now grounded (Linnaeus + the device scroll-dependence repro) as an iOS compositor scroll-collapse snap, NOT the snapshot-teardown the 6g note supposed: `holdGhostUntilPaintable` (js/app.js) gains a third async gate `settled` (`decoded && painted && settled`), armed ONLY on a large outgoing clamp (`cur.scroll0 > SETTLE_SCROLL_MIN = 0.5·innerHeight`) and released by a `window` `scrollend` (primary) or a bounded `SETTLE_MS = 100ms` backstop, with the pre-existing 600ms DIRECT `drop('timeout')` as the never-strand net; home-scoped via a `{scrollSettle}` flag at the single commit→home call so abort→browse stays byte-unchanged, and CONDITIONAL on the clamp so a small/top reveal keeps its pre-6h fast path (Loki regression fix); NEW POLICY (EC §4.19), no known-red; classification is CONDITIONAL — principled IF `scrollend` fires on the instant programmatic scroll, else a bounded heuristic hold, told apart by a `via=`/`settle=` device log; efficacy is DEVICE-ONLY (the scroll-down repro), the CI guarantee is the mechanism + never-strand + bounded listener only, so the stage is NOT "confirmed fixed" until the device repro is clean — see PLAN-swipe-stage6h.md. STILL DEFERRED on the Home-flash axis: the incoming-`#browse` headline flash (browse→browse, home→browse — T8-forked reveal-centralization, `#browse` un-promoted); the commit books→home flash (flash A) is no longer fully deferred — 6h ships the device-pending scroll-settle fix above, awaiting the device repro. DEFERRED to 7 (each behind the consumer that does not yet exist): F(release) — the paint-gated `pane.release()` half (the flash core, = the I10/I17 reveal centralization); the rest of the finalization plan (commit/abort-scroll/stackEffect/reveal + the unified `planFor()` wrapper); `sourceHost`/`destinationHost`/`sameBrowseHost` host fields; the full pane object and the remaining `dispose(reason)` enum members; PANE-OWNING supersession at the SETTLING/REVEALING phase (home↔browse, →home); the `recoverSession` pre/post-stack matrix; the I12 null-on-retire half; and a production guard for the unguarded "every connected `.nav-ghost` is an owned-pane mover" invariant (Loki `STRIKE-swipe-stage6e-r1.md` residual 2, routed to a plan amendment); the browse→home OUTGOING transform (its commit takes the home-reveal HOLD path, deferred to keep 6f off the hold surface); and the INCOMING real-`#browse` transform (browse→browse headline [T8-forked], home→browse, overlay→browse), whose transform-elimination needs workstream C plus a non-T8 incoming representation — see PLAN-swipe-stage6e.md §10 and PLAN-swipe-stage6f.md §10.]
   7. Replace Browse hold calls with the lease interface. Detail:
      Claude/Plans/PLAN-swipe-stage7.md — whose §3 VERIFIES stage 6's
      DEFERRED-to-7 clause against current source, re-derived independently
      by the plan reviewer: SIX of its ten items have no subject in the
      tree (the de-clone removed every owned pane; two of the six had their
      premise inverted), one is already built, and one — sameBrowseHost —
      is a defended refusal with a live mutant, not a deletion. So stage 7
      is the Browse hold boundary and nothing else; three items are
      re-deferred there with their consumers named. §3.5's `ready: Promise`
      and `settledScrollY` are REFUSED as dead fields, and §3.5's
      `Swipe.debugSnapshot`/`debugTrace` trace DOES NOT EXIST at HEAD — the
      stage builds one line of it rather than citing it. §14 of the
      subtraction plan's mover-lifetime invariant is OWED and lands here.
