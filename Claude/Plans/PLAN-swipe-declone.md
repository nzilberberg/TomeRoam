...
| 10a | **Park-geometry probe, before the device sees it.** On the real-engine instrument, compare the resolved box of a `.browsepage` with and without `.parked`, and measure the reveal delta on a mid-park content mutation. **Both must read 0.** A non-zero result stops the sequence — do not proceed to 10b, because the abort-repaint symptom then has three candidate causes and the device gate cannot separate them (§18 round 1 F5). The box-equality half already reads 0 on three axes (round-2 measurement); the reveal-delta half is what remains owed. | the deriver | **PASSES 2026-08-01** — box delta 0/0/0, reveal delta 0, on the SHIPPED rule in `chrome --headless=new`. Both controls fired (pre-rework `top:0` shape reads +164; `overflow-anchor:none` reads −80), so the zeros are measurements, not silence. `Claude/Linnaeus/PROBE-park-geometry-2026-08-01.md`. **Step 10b unblocked.** |
| 10b | **DONE 2026-08-04 — ALL SIX ITEMS PASS** on build `2026-08-03.306` (`Claude/Zelda/DEVICE-GATE-swipe-declone-stage2-2026-08-04.md`). This was the campaign’s only unrun device gate and the subtraction plan’s risk 3; it did not fail, so no branch is being kept as insurance. **Device gate, on the form that ships.** `browse→browse` both directions, commit and abort, on a long list and a short one — **and the abort must return to the page it started on** (§5.3.6, `LANDEDPAGESHOWS`'s device half); **at drag start the outgoing page must not jump off-screen** (§5.3.6's named observation); browse re-entry after leaving to Home keeps its scroll position (§18 round 2 F15); the A–Z strip during a `browse→browse` and a `browse→home` drag; a virtualized list past 600 items; re-confirm `home→browse`, `browse→home`, `browse→overlay`, `overlay→browse` commit and abort. **A Stage 1 device pass is not evidence about Stage 2 — Stage 2 touches the element all four of those use as a mover.** | the user | done |
| 11 | **Stage 2 subtraction pass.** Execute the remaining §12 deletion list: the now-dead pane machinery (`dropPanes`, `holdGhostUntilPaintable`, `revealPending`, the abort held-reveal branch, the `owned-pane` filters, the capture-recording block and its diagnostic readers, the `.nav-ghost` sweeps and the `keepGhosts` parameter, the `env.scrollY` supplier) and the remaining test and tooling entries. **Exit condition: every item is listed with the reason it is unreachable at step 10's HEAD** — no caller, or a caller whose condition is now constant-false. An item that cannot be shown unreachable is not a subtraction and does not belong in this step. Bump the build number. **DESIGNED IN `Claude/Plans/PLAN-swipe-declone-stage2-subtraction.md` (2026-08-04, PLAN_READY):** it carries the per-item unreachability proofs, the inverted Coverage Model, one cascade §12 does not name (the orphan-recovery collapse), and a correction to item 14's `nav.js:105`/`:106` citation. §12 remains the authoritative inventory. | the builder | open |
...
| Now Playing's uniqueness decision and its scope | read `Claude/Decisions/DecisionLog.md:1159-1179` and `:1195-1213` | confirmed — untouched by this plan |
