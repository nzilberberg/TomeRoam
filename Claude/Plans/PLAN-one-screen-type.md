...
<!-- vitruvius-gate {"plan_type":"refactor",
  ...
  "blocking_questions":["ONEPAGE","PEERPARK","PEERFINALIZE","FILMSTRIPDRAG","NPPARKS","NPRECONCILE","NOSETTINGSBG","NPUNTOUCHED","NPHIDDENWRITER","OVERLAYISNP","KINDPLAN"]} -->
...
| A1-fix, A1-fix-r2 | No separate plan-review. Gated instead by the adversary: `Claude/Loki/STRIKE-one-screen-type.md` **KILLED** the A1-fix predicate; r2 (§5.4) is the replacement. | Both **SHIPPED** — `.282` and `.284`. **Device gate step 6f is DISCHARGED**, 2026-07-31 at `de84349`, with its residual recorded: the deliberate 125–340ms flick release is **untested rather than proven clean** (§13). |
| **A1b** | Round 1: `Claude/Charpy/PLAN-one-screen-type-A1b-charpy.md`, commit `35f0005`, 2026-07-31 — **TEMPER**, six Structural findings (F1–F6), **all six verified resolved at round 2**. Round 2: `Claude/Charpy/PLAN-one-screen-type-A1b-charpy-r2.md`, commit `607d8b9`, 2026-07-31 — **TEMPER**, three new Structural (F14–F16) and six Weak/Note (F17–F22), folded 2026-07-31. Round 3 (the verdict of record): `Claude/Charpy/PLAN-one-screen-type-A1b-charpy-r3.md`, commit `20c1663` — **FORGE**. | **SHIPPED** `e6a2f2e`, build `2026-07-31.290`. Code review `Claude/Poirot/POIROT-one-screen-type-a1b-e6a2f2e.md` — **PASS, fix-then-ship** (four records-only findings, applied). Adversary `Claude/Loki/STRIKE-one-screen-type-a1b.md` — **HELD_STONE**. Coverage audit: round 1 `Claude/Mendeleev/AUDIT-one-screen-type-a1b.md` — GAPS_NAMED (three bare cells); round 2 `…-a1b-r2.md`, the verdict of record — **ADEQUATE**, all three filled by `48b19f9` and each verified by an applied mutant. **Device gate step 9 — DONE 2026-08-03, all four items PASS** (`Claude/Zelda/DEVICE-VERDICT-one-screen-type-a1b-2026-08-03.md`, build `.306`). **What remains on A1b: the campaign manifest's one-token `coverage-audit` glob fix** (round 2's §5 — the gate's `verdictArtifactGlob` has no wildcard, so it cannot read the `-r2` verdict), and **§13 step 10a**. |
...
**Stage A1b is added, 2026-07-31, and is SHIPPED** — commit `e6a2f2e`, build `2026-07-31.290`, code
review PASS fix-then-ship, adversary HELD_STONE; the step-9 device gate is what remains open. A1's
transparency exposed a pre-existing
...
dep). All three `FILMSTRIPDRAG` windows are green; the settle-window mutant
`one-screen-type FILMSTRIPDRAG-c` is registered and killed by its declared cell, and the two rotting
anchors — `one-screen-type FILMSTRIPDRAG-a` and `…-b` — are re-transcribed and pass the anchors gate.
**Named rather than numbered on purpose: this sentence previously cited `#113`, `#111` and `#112`, and
all three indices now address different mutants (§14).** `js/nav.js`'s two rewritten invariant comments (the
...
|---|---|---|
| `Claude/Decisions/DecisionLog.md:1159-1179` — "ONE SCREEN TYPE, with Now Playing the deliberate exception", 2026-07-30, USER DECISION | **Governing** | Highest authority. It settles both the approach and the exception. This plan chooses only the sequence and the mechanism. **Two of its clauses bind this plan directly.** `:1174-1175` incorporates the probe's load-bearing set **by reference**, so retiring a member of that set is a change to what the decision rests on and must be visible (F2 — see the supersession row below). `:1179` states *"do not cite its background as its distinguishing property"*, which this plan violated in three places and no longer does (F3). **The entry is NOT rewritten by this plan** — amending a ratified USER DECISION is the user's call. |
| `Claude/Decisions/DecisionLog.md:1207-1225` — "A BY-REFERENCE ITEM OF THE NOW-PLAYING DECISION IS SUPERSEDED", 2026-07-31, records pointer | **Governing, and the correct citation for the superseded parts** | **AGREE, and this is the record to cite rather than the ratified entry.** It records that one member of the by-reference set is retired (4.2) and one added (1.9), and it names two sentences of the ratified entry as now stale — `:1169-1170`'s "the background is SHARED with `#options` and all five subs" (false at HEAD since Stage A1) and `:1174`'s "Thirteen" (the wrong number). **Where this plan needs either fact it cites this entry, never the stale sentences.** The three-co-required-properties derivation at `:1170-1173` holds unchanged at HEAD and is cited directly. **⚠️ Every line citation into the decision log in this plan was re-derived against HEAD `3ba4c6e` on 2026-08-04 and moved by +12** — an entry was inserted above this one after the citations were written, which is the failure mode the append-never-insert records rule exists to prevent. Other records still carry the pre-shift numbers. |
...
| Same record, §4.2 (`:115-117`) and its §8 entry | Derived fact, **ratified by reference** | **CONFLICT, resolved by dated supersession — not by assertion.** §4.2 marked the settings-loop guard (`js/nav.js:78` at HEAD) **load-bearing**, and §8 carried it into the set `DecisionLog:1174-1175` incorporates. That is the guard §12 item 26 deletes, so an earlier draft's claim that the exemption "is not on the probe's list" was **false**. Resolved at the record: **probe §9.1 retires 4.2 to `context`**, on the ground that `hidden` is added to `#nowplaying` in exactly one place (`js/nav.js:81`) and the same synchronous `setView` body clears `hidden` from the destination three lines earlier (`js/nav.js:78-80`) — so the destination is mounted at the instant NP is hidden **on every path**, and the mark's stated reason is refuted at its root. **The bound is narrow and this plan states it narrowly:** the claim was true for the *park* guard (`js/nav.js:51`, §12 item 25 — probe §4.1, marked **context**) and false for the settings-loop guard. |
| Same record, §1.9 (`:245`, superseded by §9.2) | Derived fact | **AGREE at HEAD, after a mark change in the opposite direction.** 1.9 (NP's background) was `context / not a difference — shared with six screens`, correct when derived at `288504e`. **Stage A1 removed the six shared declarations**, so probe §9.2 **promotes 1.9 to load-bearing**: at HEAD `--page-bg` is declared by `body::before` (`css/app.css:44`) and `.nowplaying` (`css/app.css:510`) alone, pinned by `test/page-bg-single-painter.test.js:28`. **The distinction this plan must get exactly right:** the background IS load-bearing, and it is STILL not NP's *distinguishing* property (`DecisionLog:1179`). It is one of **three co-required properties for covering the bars** (`DecisionLog:1170-1173`), which is a different claim. S4 states it that way. |
...
- **It was a load-bearing mark, and the USER DECISION incorporates the set containing it by reference**
  (`DecisionLog:1174-1175`). Any claim that it was never on the list is false.
...
property**, and `DecisionLog:1179` forbids citing it as one in as many words. The decision's own
framing is the one to use: an opaque background, `inset: 0` and `z-index: 60` are **three co-required
properties for covering the bars** (`DecisionLog:1170-1173`) — remove any one and the bars are
...
2. `DecisionLog:1174-1175` incorporates that set **by reference**. So the mark was ratified.
...
4. `DecisionLog:1207-1225` records the supersession against the ratified entry, and states that the
...
user "had to repeat because it kept being re-litigated" (`DecisionLog:1160`), so a stage that
...
- **`d.browseWillHide`** (`js/nav.js:60`) — the injected hook wired to `Browse.deactivate()`
  ...
  **The re-open condition, stated as two mechanical predicates rather than as a thing to remember.**
  Edge 5 stays uncovered only while its `setView` body is the same body `NPPARKS` drives. **The moment
  edge 5's path acquires an effect edge 4's path does not have** — a branch inside `setView` keyed on
  the supersession, a new `applyScreen` option that reaches the park/hide block, a caller-specific
  hook — the byte-identity ground is gone and this edge owes a cell.

  **What makes that identity true at HEAD is two source facts, and both are checkable in one line
  each.** (1) `setView` is declared with **exactly one parameter** — `function setView(v)`
  (`js/nav.js:45`) — so no caller can hand it anything that distinguishes edge 5 from edge 4. (2)
  `applyScreen`'s Now Playing branch passes it the **literal** `'nowplaying'` and nothing else
  (`js/nav.js:150` at HEAD `3ba4c6e`); `opts.render`, `opts.resetScroll` and `opts.keepGhosts` are
  consumed by `applyScreen` itself and **never reach the park and hide statements**. Together those two
  close the second and third members of the list above *by construction*, not by inspection: while both
  hold, no option and no caller identity can reach `js/nav.js:52-70`, so edge 5 and edge 4 cannot
  diverge there.

  **⚠️ Those two facts are asserted nowhere. §13 step 10a routes them to the test author** as two
  assertions inside `NPHIDDENWRITER`'s synchrony cell (`test/np-hidden-writer-set.test.js`), which
  already locates and reads `setView`'s body in `js/nav.js` and is therefore the cell where they cost
  two lines rather than a file. A mutant that gives `setView` a second parameter and passes an option
  through `applyScreen`'s NP branch is what proves them failable. **Until step 10a lands, this ruling
  is enforced by memory, and the plan's own standard is that a rule enforced by memory is vigilance.**

  **What the two predicates do NOT reach, named so their silence is not read as coverage.** A branch
  inside `setView` keyed on **module-scope state** that only the supersession path sets would leave
  both predicates green and still break the identity. That is the residual; no such branch exists at
  HEAD (`setView`'s body reads `v`, the `npOpen` it derives from `v` on its first line, the injected
  `d`, the module constant `SETTINGS_SUBS` and `document` — and nothing else), and closing it would
  need an identifier-set pin over the whole body, a heavier instrument than this ruling is worth.
  **Step 16 still re-checks the ruling; after step 10a it is no longer the only thing that does.**
...
37. **`css/app.css:508-509`** — `/* nav.js's setView(): NP is an ADDITIVE overlay that paints over a
    ...
    reason (`DecisionLog:1170-1173`): an opaque background, `inset: 0` and `z-index: 60` are what
...
| # | Step | Owner |
|---|---|---|
| 1 | **Stress this plan, PER STAGE — this step is discharged separately for each stage.** A1/A2/B: done, `e979a41`, TEMPER, folded. **A1b: round 1 (`35f0005`) returned TEMPER with six Structural findings, all folded and all verified resolved at round 2; round 2 (`607d8b9`) returned TEMPER with three new Structural (F14–F16) and six Weak/Note (F17–F22), folded. Round 3 (`Claude/Charpy/PLAN-one-screen-type-A1b-charpy-r3.md`, `20c1663`) returned FORGE — this step is DISCHARGED for A1b.** `Claude/Campaigns/one-screen-type-a1b.json`'s `plan-review` gate accepts `FORGE` alone and reads the highest `-rN` round as the verdict of record; it reads round 3's FORGE and passes. | the plan reviewer |
...
| 8 | **Stage A1b build. ✅ DONE — SHIPPED `e6a2f2e`, build `2026-07-31.290`.** Step 1's A1b re-review is filed and accepted (`Claude/Charpy/PLAN-one-screen-type-A1b-charpy-r3.md`, **FORGE**, `20c1663`; the campaign gate reads r3 as the verdict of record and passes). Step 6f is discharged at `de84349` with its residual recorded — see the sequencing rule below. **Behaviour — two deleted conditions, and nothing else:** `js/nav.js` delete both `if (!npOpen)` guards (**`:51`** and **`:78`**) so the park/hide block and the six-way loop run unconditionally. `npOpen` the variable, the `hidden` toggle (`:81`) and the `np-locked` toggle (`:82`) all stay. **Comments — six sites, zero behaviour (§12 items 27, 28, 34, 35, 36, 37):** delete `js/nav.js:71-77` **whole** (seven lines, not two); correct `js/nav.js:48-50`; correct `js/nav.js:151`, `js/app.js:1343`, `js/app.js:494-497` (**four comment lines, of which only `:494-495` are false — `:496-497` is the still-true exception clause and no fragment of the retired sentence may survive; §12 item 36**) and `css/app.css:508-509`. **⛔ `css/app.css:510`'s `background: var(--page-bg)` is NOT touched** — item 37 is the comment above it only. **Tests and tooling — execute §6a's casualty table:** de-register mutant `#104`, re-point `#106`, retire `NPUNTOUCHED`'s two class-state cells (`test/one-screen-type.test.js:196`, `:211`; their subject moves to `NPPARKS`), keep its source-scan cell (`:218`), and land step 7's relocated `PEERFINALIZE` cell. Bump the build number. | the builder |
| 9 | **DONE 2026-08-03 — ALL FOUR ITEMS PASS** (`Claude/Zelda/DEVICE-VERDICT-one-screen-type-a1b-2026-08-03.md`, build `.306`): the accumulation is gone on device; aborted NP gestures leave no stack; no cover re-decode, no restore flash and no visible compounding cost (3c hedged "seems fine" — R-H hazard 3 NOT OBSERVED, which is weaker than disproven); and the adversary’s scroll-preservation item holds on WebKit. **Device gate A1b — the NP-close path AND the aborted NP-back swipe.** Open NP from Home, from Books and from a settings screen; close each by swipe and by back. Abort an NP-back swipe and an NP→chapter-list swipe, then swipe again — the accumulation in §5.3.1 must be gone and no more than one screen may ever be visible beside NP. **Three honest questions, all three of §15 R-H:** (1) does closing NP back to Books re-decode the covers; (2) does the restore flash; (3) **on the REPEATED half-swipe back from NP — the path this step drives most — does the list get slower or emptier the more times it is repeated?** Half-swipe back from NP and abort, repeatedly, as fast as is comfortable, on a long Books list. **What R-H hazard 3 predicts is a compounding cost, not a first-pass one:** the mid-drag render is paid today and is unchanged by A1b; what A1b adds is a teardown that dematerializes the rows the *next* swipe's render must rebuild. **A fourth item, from the adversary's strike (`Claude/Loki/STRIKE-one-screen-type-a1b.md`): scroll position across the new `#browse` `display:none`.** The strike proved on a Blink bench that a scroller's `scrollTop` survives the round trip; the device is WebKit and this is untested there. **Gesture:** scroll the Books list well down, open Now Playing, close it. **Observable:** the list must be at the same scroll position it was at before Now Playing opened — if it has jumped back toward the top, that is the failure. | the user |
| 10 | **DONE** — `Claude/Poirot/POIROT-one-screen-type-a1b-e6a2f2e.md`, **PASS, fix-then-ship** (four records-only findings, applied via apply-review; two further findings routed to the coverage auditor). Review the Stage-A1b build. **The `showAppView` sweep is NOT an open question here** — it was settled by execution at the A1 review (§5.3.5, determination KEEP) and must not be re-opened. | the code reviewer |
| 10a | **Make §9's edge-5 re-open condition mechanical — two assertions, not a new cell.** §9 rules edge 5 (supersession while Now Playing is current) deliberately uncovered on the ground that its `setView` body is byte-identical to edge 4's, and that ground rests on two source facts nothing asserts: `setView` is declared with **exactly one parameter** (`js/nav.js:45`), and `applyScreen`'s Now Playing branch passes it the **literal** `'nowplaying'` and no options (`js/nav.js:150`). Add both to `NPHIDDENWRITER`'s existing synchrony test (`test/np-hidden-writer-set.test.js`), which already locates and reads `setView`'s body out of the same file — the failure message names §9's ruling and says that a green here is what keeps edge 5 uncovered. Register one mutant that gives `setView` a second parameter and passes an option through the Now Playing branch, and confirm it reddens this cell alone. **Why it is a step and not a note:** §9's condition was written to be re-checked at step 16, which runs once and then closes with the plan, so a future reader meets the ruling with nothing behind it — and this project's standard is that a rule enforced by memory is vigilance. **Residual, stated in the cell:** a branch inside `setView` keyed on module-scope state would pass both predicates; no such branch exists at HEAD and closing it would need an identifier-set pin over the whole body. | the test author |
...
**✅ DISCHARGED 2026-07-31 — this constraint no longer binds.** Step 6f is satisfied: the user
reports the app fine on a build ≥ `.284`, and app code is **byte-identical `.284`→`.288`**
(`git diff 01cbaf1 de84349 -- js/ css/` is empty), so the reading covers the shipped r2 predicate
whichever build it ran on; the sighted-once pop-in has not recurred. Recorded at `de84349`
(`Claude/Zelda/Board.md`).
⚠️ **RESIDUAL, deliberately not upgraded to "proven":** a pass in ordinary use may not have driven
the deliberate fast release, and a **held** drag cannot reach that window at all — so the
125–340ms band is **untested rather than proven clean**. It is not to be re-asked (the user has
said the pop-in is not worth chasing without a repro). **If it ever recurs, that is the repro and
this band is the first place to look.**
⇒ **Step 8 may open.** The original constraint is kept below because its *reasoning* stays true and
is what makes the residual legible — A1b widens this defect's reach if it were ever real.

**The constraint as originally written (historical, no longer binding):** A1b must not ship before
the A1-fix-r2 device gate has read. It was stated in three places for the same reason
(here, in step 8's cell, and in `Claude/Campaigns/one-screen-type-a1b.json`). **Why it bound:** r2 is
...
PEERPARK | entering any settings screen parks home and hides browse exactly as entering browse does and the browse controller deactivation hook fires on the shown to hidden edge before the hidden class lands | unit apply books then apply options against the real index fixture with a recording browseWillHide dep and assert home carries parked and browse carries hidden and the hook fired exactly once and observed browse still unhidden at the moment it ran | THREE registered mutants and two of them are needed because one cannot exercise both edges. one-screen-type PEERPARK/PEERFINALIZE-a restores the settings exemption in the park guard so home is not parked and browse is not hidden which reddens both class assertions. one-screen-type PEERPARK/PEERFINALIZE-b moves the browseWillHide call after the hidden toggle which reddens the observed-unhidden assertion. one-screen-type PEERPARK-c deletes the browseWillHide call so the hook never fires on the settings shown to hidden edge which reddens the call-count assertion. All three are SHARED with PEERFINALIZE and are counted once here | unit nav screen-state against the real index fixture
PEERFINALIZE | the narrowed park guard is correct on the GESTURE-FINALIZE path and not only on the button-nav path so a committed home to settings gesture leaves home parked and a committed browse to settings gesture leaves browse hidden with the browse deactivation hook fired exactly once while browse was still un-hidden and an aborted settings to browse gesture does the same after the mid-drag render already un-hid browse and — Stage A1b — the NP to files ABORT is where the browse deactivation edge now fires rather than later at the Now Playing close | integration boot the app harness with a recording browse deactivation dep and drive FOUR real gestures to completion — commit home to options then commit books to options then abort options to books then abort an NP to files swipe — and after each assert the home parked class and the browse hidden class and the hook call count and the hidden state observed at the moment the hook ran. The fourth scenario is §9 edge 3 RELOCATED by Stage A1b: install the recorder BEFORE the abort or the relocated call goes unrecorded and assert the count is 1 at the abort and STILL 1 after the Now Playing close so the edge is proven to have MOVED rather than duplicated | FOUR registered mutants reach this cell and TWO of them are shared with PEERPARK and counted once in PEERPARK's row — one-screen-type PEERPARK/PEERFINALIZE-a which restores the settings exemption in the park guard so neither home is parked nor browse hidden at finalize and one-screen-type PEERPARK/PEERFINALIZE-b which moves the browse deactivation call after the hidden toggle. one-screen-type PEERPARK-c deletes the browseWillHide call so the hook never fires which reddens the call-count assertions and is likewise counted once in PEERPARK's row. one-screen-type NPPARKS-a' reddens the relocated edge-3 scenario and is counted once in NPPARKS's row | integration app harness over the real shipped gesture listeners
FILMSTRIPDRAG | a pending overlayFilmstrip reconcile does not hide or un-transform an element a gesture SESSION owns as a mover across all three windows of that ownership — during the live drag and during the post-release settle phase before finalize — while an armed-only gesture that never locks still discharges the pending reconcile so the filmstrip is never stranded mid-transform | integration boot the app harness with fake timers and drive the real closeSub filmstrip three ways. window one arm and lock and hold and advance past the net and assert the incoming mover is not hidden and keeps its inline transform. window two arm without locking then release and assert the reconcile ran and no transform is stranded. window three arm and lock and RELEASE with commit before the net is due then advance past the net but not past the settle finalize and assert the committed incoming mover is still not hidden and both movers keep their settle transforms and the session still owns them then advance past finalize and assert no hidden to shown flip occurred | FOUR registered mutants and the fourth defends the safety net rather than the predicate. one-screen-type FILMSTRIPDRAG removes overlayFilmstrip's 340ms reconcile safety net so a missed transitionend leaves the filmstrip un-reconciled with the outgoing pane keeping its inline transform and the sub-screen staying un-hidden which reddens window two. one-screen-type FILMSTRIPDRAG-a is NATURAL-a and one-screen-type FILMSTRIPDRAG-b is NATURAL-b and one-screen-type FILMSTRIPDRAG-c is NATURAL-c below. NATURAL-a remove the ownership condition so the pending finish runs during the live drag which reddens window one. NATURAL-b make the condition test armed rather than live which strands the filmstrip transform and reddens window two. NATURAL-c restore the shipped drag-liveness form of the predicate so it reads the nulled drag handle instead of the session which stops suppressing after release and reddens window three | integration app harness over the real shipped filmstrip and gesture listeners
NPPARKS | entering Now Playing parks home and hides browse and hides all six settings screens exactly as entering any other screen does and fires the browse deactivation hook once on the shown to hidden edge while browse is still un-hidden | unit against the real index fixture apply books then apply nowplaying with a recording browse deactivation dep and assert home carries parked and browse carries hidden and all six settings screens carry hidden and nowplaying does not and the hook fired once observed with browse still un-hidden then repeat entering from options and from home | THREE registered mutants and the park and hide block needs two of them because one mutant cannot exercise both toggles. one-screen-type NPPARKS-a restores the npOpen guard on the PARK TOGGLE ALONE so home is not parked which reddens the from-Home cell. one-screen-type NPPARKS-a' restores it on the BROWSE-HIDE TOGGLE ALONE so browse is not hidden which reddens the from-Browse cell and also PEERFINALIZE's relocated edge 3 and NPRECONCILE. one-screen-type NPPARKS-b restores the guard on the six-way settings loop so a settings screen stays un-hidden under Now Playing which reddens the six-hidden assertion and also NPRECONCILE's settings companion and the inverted cell in test/nav.test.js. All three are SHARED with NPRECONCILE and are counted once here | unit nav screen-state against the real index fixture
NPRECONCILE | an aborted Now Playing gesture reconciles what the gesture un-hid so screens cannot accumulate underneath Now Playing across repeated aborts which is the mechanism that produced the three-screen render | integration boot the app harness and from Now Playing opened over Home drive a back-swipe to home and abort it then a forward-swipe to the chapter list and abort it and in a companion cell from the Options hub open Now Playing and drive a back-swipe to options and abort it. Assert in each cell as fixture sanity that the mid-drag destination render really did un-park home or un-hide options so the forcing is witnessed rather than assumed. Then assert ABSOLUTELY after abort 1 that home carries parked and ABSOLUTELY after the companion abort that options carries hidden. Separately read the set of screen elements besides nowplaying lacking BOTH the hidden and the parked class as an instrument reading at Now Playing's entry and assert that set is unchanged after each abort and does not grow across the two aborts. ⚠️ Under Stage A1b that set is EMPTY from a Home source because Now Playing's entry parks home and hides browse and hides all six settings screens — the number is ZERO and not one. An absolute assertion is REQUIRED beside the relative one because a relative assertion cannot fail wherever the defect is already present at entry: a defective entry moves the baseline with it. ⚠️ The companion must drive the BACK swipe. On the forward abort showAppView's mid-drag sweep hides every settings overlay that is not the gesture's own source so options ends hidden by js/app.js whatever setView did and the cell would pass vacuously | THREE registered mutants force this cell and NONE of them is its own — all three are shared with NPPARKS and are counted once in NPPARKS's row. one-screen-type NPPARKS-a reddens the absolute home re-park after abort 1. one-screen-type NPPARKS-a' reddens the relative growth assertion at the second abort. one-screen-type NPPARKS-b reddens the settings companion. ⚠️ The forcing set is stated per assertion because the relative assertions ALONE are blind to NPPARKS-a and NPPARKS-b — measured at the coverage audit where this cell passed under both | integration app harness over the real shipped gesture listeners
NOSETTINGSBG | the page background variable is painted by exactly the fixed body pseudo-element and the Now Playing rule and by no other screen rule so every screen but Now Playing is transparent | gate read the shipped stylesheet and assert the set of selectors declaring the page background variable is exactly the body pseudo-element and the Now Playing selector and separately assert each of home browse options and the five-sub group declares no background property at all | THREE registered mutants. one-screen-type NOSETTINGSBG-a re-adds the page background to the options rule so the painter set gains a member and the transparent assertion reddens. one-screen-type NOSETTINGSBG-a' re-adds it to the five-sub group so the painter set gains a member from the other side. one-screen-type NOSETTINGSBG-b deletes the Now Playing background so the painter set loses its only screen and the painter-set assertion reddens. NOSETTINGSBG-b is SHARED with NPUNTOUCHED and is counted once here | gate source scan over the shipped stylesheet
NPUNTOUCHED | Now Playing keeps every property the user decision names so its rule still declares its own background and its full-viewport inset and its z-index above the topbar and the transport | source scan read the shipped stylesheet and assert the Now Playing rule declares background var page-bg and inset 0 and z-index 60 and that the body np-locked navbar rule still raises the navbar above it. The navbar half asserts the RELATIONSHIP the ratified decision names — the navbar rule declares a z-index at all and it is strictly greater than the Now Playing rule's — with both values derived and the literal 70 reported in the failure message rather than pinned as the claim so a future re-tier of the whole stack does not produce a false red. Apply the same anti-vacuity not-null guard to the navbar rule lookup that the Now Playing lookup already carries | TWO registered mutants reach this cell and only one is its own. one-screen-type NPNAVBAR deletes z-index 70 from the body np-locked navbar rule so the navbar stops outstacking Now Playing and Now Playing becomes an ordinary screen which reddens the navbar comparison and is caught by this cell ALONE. one-screen-type NOSETTINGSBG-b deletes the Now Playing background declaration which reddens the painter assertion here and reddens NOSETTINGSBG from the other direction — SHARED and counted once in NOSETTINGSBG's row. ⚠️ The other three assertions on the Now Playing rule — position fixed and inset 0 and z-index 60 — carry NO registered mutant and the cell says so in its own text | gate source scan over the shipped stylesheet
NPHIDDENWRITER | the hidden class is added to nowplaying by exactly ONE first-party code path in js/ and the same synchronous setView body un-hides the destination before it — this is claim C6 and it is what licenses retiring the ratified probe mark 4.2 so it is the whole difference between Stage A1b being a licensed change and an unlicensed one | gate derive over first-party js/ with the vendored tree excluded BY REFERENCE to the existing content-hash identity pin and with comments stripped and strings respected. Five derivations. (1) an IDENTITY inventory of every code site naming the nowplaying element by quoted id or by selector each registered with a role of which exactly ONE carries the hidden-write role. (2) a HIDDEN-STATE WRITE inventory over classList add and toggle with the hidden LITERAL and style display assignment and the hidden property assignment and setAttribute class with each site's receiver classified — an id literal or an id loop variable is resolved MECHANICALLY while a local binding or a query result is REGISTERED with its target and owner and a one-line reason it is not nowplaying and an unresolved id variable or anything unclassified FAILS. (3) an ALIAS closure so no write of any of the five routes INCLUDING className assignment may have as its receiver an identifier bound to the element. (4) a SPREAD closure so a registered loop list that spreads a named constant has that constant's own array literal checked for the token behind an anti-vacuity guard. (5) the SYNCHRONY half asserting the browse toggle and the six-way settings loop both precede the nowplaying toggle inside setView's body and that the body contains no await and no requestAnimationFrame and no setTimeout and no queueMicrotask and no then. Assert in THREE directions: an unregistered derived site fails and a registered entry whose text no longer occurs fails and a group whose derived count differs from its registered count fails. Anti-vacuity: the file set must exceed twenty files and both inventories must find a non-zero number of sites. ⚠️ State the residual bound IN THE CELL because this gate's silence must not be readable as coverage — it is the TEXTUAL bound and a receiver resolved at runtime and a write through Object.assign and a class added from a variable are all outside its reach | FOUR registered mutants each of which must be BEHAVIOURALLY INERT so that nothing else can claim it and each caught by this cell ALONE. NPHIDDENWRITER-a injects a second redundant hidden writer on nowplaying into setView which ONLY the group-count direction can see because a duplicate matches its registered entry textually. NPHIDDENWRITER-b makes the nowplaying alias write the hidden class which is the one route that reaches the element without naming it. NPHIDDENWRITER-c widens showAppView's stale-overlay sweep BY ONE WORD to include nowplaying which is the coverage audit's named highest-probability next defect on this surface. NPHIDDENWRITER-d reorders setView so nowplaying is hidden BEFORE the destination is un-hidden which leaves the writer count unchanged and breaks C6 anyway | gate source-derived inventory over first-party js/ — a LOCK that is green at HEAD by design rather than a red cell
...
**The mutant accounting, stated once and checkable against the rows.** Eleven cells. **Nine are
shipped and carry NINETEEN distinct registered mutants between them**; two — `OVERLAYISNP` and
`KINDPLAN` — are Stage B, and their mutants are registered at that stage's build (§13 step 13). Each
row's mutation column lists its own mutants by name, so the total below is *checked* against the rows
rather than asserted beside them.

| Cell | Its registered mutants, by name | Distinct entries first counted here |
|---|---|---|
| `ONEPAGE` | `one-screen-type ONEPAGE` | 1 |
| `PEERPARK` | `one-screen-type PEERPARK/PEERFINALIZE-a`, `…-b`, `one-screen-type PEERPARK-c` | 3 |
| `PEERFINALIZE` | those same three, **shared**, plus `one-screen-type NPPARKS-a'` (counted under `NPPARKS`) | 0 |
| `FILMSTRIPDRAG` | `one-screen-type FILMSTRIPDRAG` (the 340ms net), `…-a`, `…-b`, `…-c` | 4 |
| `NPPARKS` | `one-screen-type NPPARKS-a`, `…-a'`, `…-b` | 3 |
| `NPRECONCILE` | those same three, **shared** — it has no mutant of its own | 0 |
| `NOSETTINGSBG` | `one-screen-type NOSETTINGSBG-a`, `…-a'`, `…-b` | 3 |
| `NPUNTOUCHED` | `one-screen-type NPNAVBAR`, plus `one-screen-type NOSETTINGSBG-b` **shared** | 1 |
| `NPHIDDENWRITER` | `NPHIDDENWRITER-a`, `…-b`, `…-c`, `…-d` | 4 |
| `OVERLAYISNP`, `KINDPLAN` | none — Stage B, registered at step 13 | 0 |
| **Total distinct** | | **19** |

**The count this table replaces was seventeen, and it was right only by coincidence.** Four registered
mutants appeared in no row (`NPPARKS-a'`, `NOSETTINGSBG-a'`, `PEERPARK-c`, and the `FILMSTRIPDRAG`
340ms net) while three were counted twice (the `PEERPARK`/`PEERFINALIZE` pair as two pairs, and
`NOSETTINGSBG-b` once as `NOSETTINGSBG`'s and again as `NPUNTOUCHED`'s), and the two errors cancelled.
A total that reconciles while its parts do not is the shape to distrust: the parts are now enumerated,
and **shared mutants are named in both rows and counted in exactly one.**

⛔ **Mutants are cited in this plan by REGISTRY NAME and never by registry index.** `tools/mutate.mjs`
is an ordered array, so inserting an entry shifts every later index with no error and no diff signal:
registering `one-screen-type NPNAVBAR` ahead of the `FILMSTRIPDRAG` block moved the 340ms safety-net
mutant from `#95` to `#100`, and records written before that insert kept citing the old number. Derive
an index at the moment of use — `node tools/mutate.mjs --list` — and never write one into a record.

**Every cell asserts a source fact, a class-state fact, a call-count
...
⚠️ **The ruling is CONDITIONAL, and until §13 step 10a lands, the condition is enforced by memory
alone.** §9's re-open condition was written to be re-checked at step 16 — a step that runs once, at
the end of Stage B, and then never again. A ruling whose only trigger is a step that closes with the
plan is a ruling a future reader will find stated as permanent. §9 now states the condition as two
mechanical predicates over `js/nav.js`, and **step 10a routes them to the test author as two
assertions in `NPHIDDENWRITER`'s existing synchrony cell** — the cell that already reads `setView`'s
body out of the same file. Until they land, this row reads: **edge 5 is uncovered on a ground that
nothing checks.**
