...
- **NEXT ACTION — navstack plan review ROUND 2, dispatched.** Round 1 was TEMPER (`596c579`, four
  Structural); the temper is applied (`6730c8b`, plan only). The design and the standalone-and-first
  ruling survived both rounds.
...
| T-S7O | ⚠️ Two items the navstack plan does NOT cover, both carried forward deliberately: §1's branch-B **chapter-list** throw was never re-executed (fixture needs library data; drive S drives the back branch instead), and `NAVTOTAL` is **source-only** — deleting both branch conjuncts at once is indistinguishable from the specified predicate across drives F, I and S | the plan reviewer | OPEN 2026-08-06 | round 2 judges whether drive S substitutes for branch B, and whether source-only `NAVTOTAL` is honest or is round 1's F4 returning under another name | none | `Claude/Plans/PLAN-swipe-navstack-settle-window.md` §1, `NAVTOTAL` |
...
| T-TOOL3 | ⭐⭐A plan's blast radius gets measured from a SUBSET of the plan's own declared edits, and the plan states the figure without stating which edits produced it. **STRUCK TWICE**: stage 7 r1 F1 (measured from a narrower transform than §5 declared) and navstack r1 F1 (measured from 1 of 3 edits; 3 registrations refused, not 1) | the planner | OPEN 2026-08-06, filed on the second occurrence | build the check: the declared blast radius names the edit set it was measured from, and that set equals the plan's declared edit set. Until then §13-style equality rules halt builders on correct work | none | `Claude/Charpy/PLAN-swipe-navstack-settle-window-2026-08-06-r1.md` F1; `Claude/Charpy/PLAN-swipe-stage7-2026-08-06-r1.md` F1 |
