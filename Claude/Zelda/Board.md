...
| T-TOOL3 | ⭐⭐A plan's blast-radius equality rule fires on CORRECT work, because the declared figure and the gate's check are taken against different inputs. **STRUCK THREE TIMES, two distinct sub-causes.** (a) measured from a SUBSET of the plan's declared edits — stage 7 r1 F1 (a narrower transform than §5 declared) and navstack r1 F1 (1 of 3 edits; 3 refused, not 1). (b) measured against a different TREE STATE than the gate evaluates — navstack r2 F10: step 5b demands exactly 1, but against the BUILT tree, which carries the re-anchoring, it is 0, so "a smaller set is also a stop" halts a correct build | the planner | OPEN 2026-08-06, widened on the third occurrence | build the check: a declared blast radius names BOTH the edit set AND the tree state it was measured against, and the gate evaluates the same two. Both sub-causes are silent — each halts a correct build at its last step | none | `Claude/Charpy/PLAN-swipe-navstack-settle-window-2026-08-06-r2.md` F10; `…-r1.md` F1; `Claude/Charpy/PLAN-swipe-stage7-2026-08-06-r1.md` F1 |
