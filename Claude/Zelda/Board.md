...
- **NEXT ACTION — the planner applies the navstack round-1 temper (`T-S7M`)**, TEMPER at `596c579`,
  four Structural findings. The design and the standalone-and-first ruling both survived.
- ⚠️ **Stage 7 owes a PER-RANGE re-measure once the navstack slice lands, not a constant shift.**
  MEASURED by the reviewer: `1022-1026` shifts **+4** but `1071-1081` shifts **+8**. The navstack
  plan's claim of a constant shift is wrong and is part of the temper.
...
| T-S7M | Settle-window nav-stack corruption in SHIPPED code: a nav tap during the ~340 ms settle window empties the stack the committing gesture is about to mutate | the plan reviewer | PLANNED 2026-08-06 (`13a97b0`), REPRODUCED with passing negative controls — and the strike's report was INCOMPLETE. The **back branch has the same defect** (`navStack.pop()` at `js/app.js:703` is unguarded; verified unguarded at source by the assistant, reproduction is the planner's claim), and two further cases corrupt the stack **without throwing** — the throw is the loud instance of a quiet class. Ruled STANDALONE, landing BEFORE stage 7, on measured-disjoint edit ranges | round 1 plan review, full scope, dispatched | none | `Claude/Plans/PLAN-swipe-navstack-settle-window.md` |
