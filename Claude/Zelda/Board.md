...
- **Parked-page-rides-Home fix — ROUND-2 TEMPER (2026-08-02), back with the planner:**
  `Claude/Plans/PLAN-parked-page-rides-home.md`, reviews at
  `Claude/Charpy/PLAN-parked-page-rides-home-charpy.md` and `…-charpy-r2.md`. One CSS declaration —
  ...
  **Round 2 (2026-08-02) — TEMPER again, two Structural, both "a cell that cannot fail":** **F9** —
  ...
  (`js/app.js:2774` → `js/nav.js:153`), so R6 files a present path as a future one. Next owner:
  Vitruvius (the two mutants only); then Curie, then Brunel. Device-owed, unchanged: cover retention at
  the new distance, and whether this is the whole of the reported garbage.
