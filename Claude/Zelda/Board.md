...
| T-S7A | Swipe stage 7 (the Browse lease boundary) — round 1 TEMPER APPLIED, re-filed for round 2 | the plan reviewer | AMENDED 2026-08-06: scope determination HELD unchanged; F1 re-measured (13 registrations in 3 classes, not 1), F2 resolved by scoping the trace line in, F3's vacuous check repaired with a `to`-side discriminator; F4/F5/F7/F8 folded; the four blocking questions became decisions D1-D3 and open-unknown U1 (adversary) | temper this revision | none | `Claude/Plans/PLAN-swipe-stage7.md`; `Claude/Charpy/PLAN-swipe-stage7-2026-08-06-r1.md` |
| T-S7E | §11's MEASURED co-change set is wrong: it was measured from a transform narrower than §5 declares. MEASURED at `79840d7`: the §5 rename rots 9 registrations (not 1), the §12 freeze rots 3 more, plus the `ADAPTER_DECL` fixture-sanity reader | the planner | OPEN 2026-08-06, blocking the build | re-derive the declared set from the transform §5 actually specifies, including the mutants' `to` sides which inject `dropRowHold()` | none | `Claude/Charpy/PLAN-swipe-stage7-2026-08-06-r1.md` F1 |
| T-S7F | The release status has no production consumer: `Swipe.debugSnapshot`/`debugTrace` do not exist in `js/` and no `PBDebug` line mentions the hold, so the "retained" trace is a future deliverable | the planner | OPEN 2026-08-06, blocking the build | either emit the trace line in this slice with a cell asserting it, or take the plan's own fallback and drop the status (EC §4.15) | none | `Claude/Charpy/PLAN-swipe-stage7-2026-08-06-r1.md` F2 |
| T-S7G | R7's mitigation is vacuous: clause 3's `registered` check is satisfied by the same commit's re-anchoring of `#147`/`#148`/`#151`, so §14's wrapper-deletion mutant can go missing with the suite green | the planner | OPEN 2026-08-06 | make the check distinguish the wrapper-deletion mutant from a re-anchored key-set mutant on the same expression | none | `Claude/Charpy/PLAN-swipe-stage7-2026-08-06-r1.md` F3 |
...
| T-S7E | The stage-7 co-change set is RE-MEASURED: 13 registrations in 3 classes, not 1 | the planner | DONE 2026-08-06, control-first, in memory; the first draft measured a narrower transform than its own scope declares | builder runs the same three-class probe at step 5b | none | `Claude/Plans/PLAN-swipe-stage7.md` §11 |
| T-S7F | `MOVERFROZEN`'s registration check needs a `to`-side discriminator, or this commit's own re-anchoring makes it vacuous | the test author | SPECIFIED 2026-08-06; the "no new mechanism is needed" claim is retracted | land the repaired predicate in the same commit as the freeze | stage 7 build | `Claude/Plans/PLAN-swipe-stage7.md` §12 |
| T-S7G | `tools/mutate.mjs:432-438` — the `swipe4 no-dead-fields` mutant's comment still says `sameBrowseHost` is "STILL unconsumed until stage 6"; stage 6 has passed and the field's planned consumer was retired | the assistant | OPEN 2026-08-06 | scrub the rationale; the mutant itself is live and correct | none | `Claude/Plans/PLAN-swipe-stage7.md` §11 |
