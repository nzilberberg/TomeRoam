...
| T-S7A | Swipe stage 7 (the Browse lease boundary) — plan review | the planner | ROUND 2 TEMPER 2026-08-06 (`c2369f8`): F1 repaired with a Weak residual, F2 repaired, F3 NOT repaired (two Structural defects). Scope determination not re-struck — it held under independent re-derivation at round 1. No round 3 owed if the amendment stays confined to three edits | apply the round-2 temper, then the adversary for §17 step 2 | none | `Claude/Plans/PLAN-swipe-stage7.md`; `Claude/Charpy/PLAN-swipe-stage7-2026-08-06-r2.md` (r1 is the prior round) |
| T-S7E | Stage 7 F1 — §11's co-change set. Round 1: measured from a transform narrower than §5 declares (9 rotted, not 1). Round 2: the re-measurement reproduces exactly and step 5b's equality rule is satisfiable by correct work | the planner | REPAIRED 2026-08-06, one Weak residual: the declared reader class is 1 of a measured 10. Eight are test files the suite catches loudly; `tools/gen-swipe-model.mjs` + `docs/swipe-model.generated.txt:144,148` are caught by nothing, because `test/swipe-model.test.js` compares generator against generated and both rot together green | name class (c) and the scrub list in §11 | none | `Claude/Charpy/PLAN-swipe-stage7-2026-08-06-r2.md` F1 |
...
| T-S7G | Stage 7 F3 — clause 3's `registered` check. Round 1: made vacuous by the same commit's re-anchoring. Round 2: the `to`-side repair does NOT close it | the planner | NOT REPAIRED 2026-08-06, two Structural defects. (a) §12 states as *verified* that the three MOVERSHAPE `to` strings keep `Object.freeze(`; measured, none of the three does, so the property is an unstated obligation on the builder's edit and the vacuity returns through a different door. (b) `NATURAL-b` cannot kill — deleting a conjunct from a `some()` can only make it more true, so it must be a two-part mutation | state the `to`-side obligation with a fourth step-5b check; make `NATURAL-b` two-part | none | `Claude/Charpy/PLAN-swipe-stage7-2026-08-06-r2.md` F3 |
...
| T-S7I | `test/swipe-model.test.js` cannot detect prose rot in the model it guards: its comparison is generator-versus-generated, so both drifting together is green by construction | the coverage auditor | OPEN 2026-08-06, not routed — a gap in a gate stage 7 merely exposes, and it outlives stage 7 | ask the user whether to pursue; it is the mechanism behind T-S7E's ungated half | none | `Claude/Charpy/PLAN-swipe-stage7-2026-08-06-r2.md` "Outside scope, surfaced not struck" |
