...
- **NEXT ACTION — the planner, on `T-S7M`**, a defect in SHIPPED code the strike found in passing:
  a forward-swipe commit whose `fwdStack` empties during the settle window throws in `runFinalize`
  and corrupts the nav stack. Taken ahead of §17 step 3 because it is live user-facing breakage
  while stage 7 is an unbuilt refactor. The planner also rules on fold-in vs standalone.
...
| T-S7M | Forward-swipe COMMIT whose `fwdStack` is emptied by a nav tap during the ~340 ms settle window pushes `undefined` onto `navStack` and throws in `runFinalize` at `js/app.js:1021` (`dest.v`): reveal report lost, commit's screen reconcile skipped, nav stack corrupt. The row hold still releases via the `finally` | the planner | OPEN 2026-08-06 — found in passing by the strike, filed un-prosecuted; a defect in SHIPPED code, carried by neither plan-review round | reproduce it, then plan the fix and rule on fold-into-stage-7 vs standalone | none | `Claude/Loki/STRIKE-swipe-stage7-lease-exits-2026-08-06.md` §5 |
