...
- **NEXT ACTION — the code reviewer**, then the coverage auditor. 3 of 5 manifest gates cleared.
...
| T-TOOL3 | ⭐⭐⭐A plan's blast-radius equality rule fires on CORRECT work, because the declared figure and the gate's check are taken against different inputs. **STRUCK FOUR TIMES, two sub-causes, and the fourth got past all three review rounds into the BUILD.** (a) measured from a SUBSET of what the plan itself mandates — stage 7 r1 F1 (a narrower transform than §5 declared); navstack r1 F1 (1 of 3 edits; 3 refused not 1); **and at build time, §8's probe transformed only the 3 CODE edits and never the mandatory COMMENT SCRUBS the same commit requires — 2 refused, not the declared 1, the extra being `EMPTYAFTERHOME-a`.** (b) measured against a different TREE STATE than the gate evaluates — navstack r2 F10 | the planner | OPEN 2026-08-06, widened on the fourth occurrence | build the check: a declared blast radius names BOTH the full change set (code AND comments AND records) and the tree state it was measured against, and the gate evaluates the same two. ⭐The fourth occurrence proves review rounds do not catch this class — only execution does | none | `Claude/Brunel/swipe-navstack-settle-window-build.md`; `Claude/Charpy/PLAN-swipe-navstack-settle-window-2026-08-06-r2.md` F10 |
