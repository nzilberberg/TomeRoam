...
**Handoff packet: `Claude/Zelda/HANDOFF.md`** — written to be picked up *without* the
...
- **`main` == `origin/main`, tree clean · build `2026-08-05.2` · suite 900 / 899 pass / 0 fail /
  1 skip · **14 of 15 campaigns COMPLETE**, the 15th being `swipe-navstack`, IN_PROGRESS with 3
  gates owed.** Re-executed and confirmed 2026-08-06: the suite via `node --test "test/*.test.js"`,
  the manifests via `tools/campaign/stage-gate-check.mjs` over every file in `Claude/Campaigns/`.
  ⚠️ **12 of the 13 skips are the navstack red suite awaiting its build** — they are not a backlog,
  and the builder lifts them FIRST. The one long-standing skip is the device-only `KEEPER` cell.
  Suite went 887 → 916 (the two board gates) → 935 (the red suite).
...
- ✅ **BUILT AND GREEN** (`8acbdff`, build **`2026-08-05.3`**). Red-first reproduced exactly before
  any code changed (19 / 7 pass / 12 fail on unmodified HEAD); all ten mutants applied, tested and
  restored individually in the foreground, each firing on its designated cell; registry 152 → **161**;
  no `*.mutbak`. Suite **935 / 934 pass / 0 fail / 1 skip** — every red cell green, and the one
  remaining skip is the pre-existing device-only `KEEPER`.
- ✅ **Code review PASS — fix-then-ship** (`e80fcbe`). **4 of 5 manifest gates cleared**; only the
  coverage audit remains, and it is deliberately NOT dispatched while the code still changes.
- **F1 was a real shipped defect and is AMENDED, not patched** (`a630d40`): `applies` is
  `commit && …`, so on every abort it was false by construction and the settle line logged
  `nav=superseded` — asserting a supersession that never happened on roughly half of all lines. In a
  project whose hardest rule is *device log before diagnosis*, a lying diagnostic corrupts the one
  instrument that settles device questions. Now a **three-arm** token (`abort` / `applied` /
  `superseded`); suppression was rejected because an absent token cannot be told from an old build.
  ⭐ Its mutant `NAVTOKEN-a` applied to the corrected source reproduces `e80fcbe`'s `js/app.js`
  **byte-for-byte** — the mutant IS the defect.
- ✅ **F1's fix is BUILT and DELTA-REVIEWED.** Curie's abort-token cell (`a509115`), Brunel's
  one-line build (`9506f3a`, build **`2026-08-05.4`**, registry **162**), Poirot's delta review
  (`0a19f59`, **PASS**). Suite **936 / 935 pass / 0 fail / 1 skip** — back to the single
  pre-existing device-only `KEEPER` skip.
- ⭐⭐ **The delta review was NOT in the plan's sequence and it found a Significant defect.** §13 put
  the coverage audit next; the user asked whether Poirot should come after Brunel. Production code
  had changed since Poirot's verdict on `8acbdff` and no reviewer had seen it — and the manifest
  could not catch that, because the code-review gate already read ✅ from the first review.
  **A cleared gate does not stay cleared when the code moves under it.**
- ✅✅ **THE NAVSTACK SLICE IS COMPLETE.** All five gates cleared — Charpy FORGE, Curie
  RED_SUITE_READY, Brunel BUILD_GREEN, Poirot PASS (twice: `8acbdff` and the delta on `9506f3a`),
...
- ⚠️ **Stage 7 owes a PER-RANGE re-measure once the navstack slice lands, never a constant shift.**
  ...
  judging which holds; treat neither as settled until it returns.
- ⛔ **Every settle-path oracle is suspect** (`T-S7N`) — the strike's "no `@reveal` report ⇒ it
  threw" proved nothing. Prove an oracle can FAIL before crediting its silence.
...
| T-S7K | `LEASEINVALID`'s supersession route may be undrivable — surfaced by the plan reviewer, deliberately NOT struck | the test author | ANSWERED 2026-08-06 by the strike (`d0201d7`): a superseded session's finalize never calls release at all, so `'invalidated'` is producible only via the `dropHold` / `invalidateGestureHold` family | brief the test author with this when §17 step 3 opens | none | `Claude/Loki/STRIKE-swipe-stage7-lease-exits-2026-08-06.md` |
| T-S7L | U1 — is the `LEASEPAIRED` exit set complete? | — | CLOSED 2026-08-06 by EXECUTION (`d0201d7`), verdict HELD STONE. Derived blind from source, then driven against the real `js/app.js` + real `js/browse.js`: 14 probe planes plus an 8-seed / 400-action interleaving fuzz. Every live gesture paired exactly one acquire with one effective release; no exit leaked; non-live exits acquired nothing. Instruments validated first — a deliberately-leaked hold registered before any verdict was read off silence. The independently-derived exit set matches both prior readings exactly, so U1 now rests on three readings **and** an execution | none | none | `Claude/Loki/STRIKE-swipe-stage7-lease-exits-2026-08-06.md` |
| T-S7M | Settle-window nav-stack corruption in SHIPPED code: a nav tap during the ~340 ms settle window empties the stack the committing gesture is about to mutate | the plan reviewer | ROUND-1 TEMPER APPLIED 2026-08-06 (`6730c8b`, plan only — registry still 152 at HEAD, so its 152→161 is a BUILD declaration not a change made). F1 resolved structurally by prescribing all three edits verbatim so the character-identical collision with `js/app.js:1038` cannot be written; F2 re-measured; F3's "silently fails to arm" corrected to the measured form (it arms, and fails only after one further navigation); F4 given a WITNESS — two new drives, I (same-view tab re-tap) and S (settings ‹ Back), where identity-deleted lands wrong on both. Ruled STANDALONE, landing BEFORE stage 7 | round 2 review, judging the dispositions + the two uncovered items | none | `Claude/Plans/PLAN-swipe-navstack-settle-window.md` |
| T-S7O | The navstack plan's two "not covered" items — branch B's throw, and `NAVTOTAL` as source-only | the planner | BOTH DISSOLVED 2026-08-06 by round 2 (`decfbd9`), and both the wrong way round from what the plan claimed. Branch B's throw IS drivable at HEAD with **no library data** (1 throw, control 0), so `NAVSTALE-b`'s red-first never waited on a fixture — and drive S produces **0** throws, so it was never a throw substitute. `NAVTOTAL` is **not** source-only: a settle-window navigation PAIR (`openSub` then `closeSub`) leaves `navStack`'s top identity intact while replacing `fwdStack`'s top, so deleting the forward conjunct alone lands the user on a third screen. The branch conjuncts are NOT redundant | fold both into the plan as F9/F11 of the round-2 temper | none | `Claude/Charpy/PLAN-swipe-navstack-settle-window-2026-08-06-r2.md` F9, F11 |
| T-NS-COV1 | ⛔⛔ A CREDITED CELL THAT CANNOT FAIL: `NAVAPPLIES (newNav)` asserts the branch clears `fwdStack`, but deleting `fwdStack.length = 0` reddens **no behavioural cell** — only the anchor gate and two line-count model pins. Instrumented reachability: the newNav branch is reached **1× in 936 tests with `fwdStack` already empty, 0× non-empty**; the forward-replay branch was hit 7× as a positive control. ⭐The marker had to be assembled at runtime, because a literal one drew 2 false hits from `swipe-model.test.js` echoing source text | the planner | OPEN 2026-08-07, found by the coverage audit reading its holes forward | rule which disposition holds: if the state is production-reachable it is a Curie cell whose missing property is the PRECONDITION (`fwdStack` non-empty at an NP→chapter-list commit), acceptance being that the transform then reddens a behavioural cell; if unreachable, §9 4(a) must stop claiming it and the statement becomes a dead-surface lead for Poirot | none | `Claude/Mendeleev/AUDIT-swipe-navstack-settle-window-2026-08-07.md` |
| T-NS-COV2 | §9 dimension 8 names "a browse→browse pair" and assigns it no cell; `landed()` collapses every browse page to `browse`. Dimension 4(a) is PARTIAL; dimension 8 was swept for five of its six named occupants | the planner | OPEN 2026-08-07, filed with T-NS-COV1 and subordinate to it | fold into the same ruling — either specify the cell or record why none is owed | none | `Claude/Mendeleev/AUDIT-swipe-navstack-settle-window-2026-08-07.md` |
| T-NS-F2 | ⛔ `Claude/Subsystems/swipe-reveal.md` item 12 STILL asserts two retired mechanisms — "release panes after the paint barrier" (no pane machinery exists in `js/`, only tombstone comments) and "abort re-renders the source into `#browse`" (`js/app.js:1054` is `render: false`) — **inside the very sentence rewritten to fix a records defect**. Verified at source by the assistant | the builder | OPEN 2026-08-07, Significant, found by the delta review | rewrite item 12 to current truth; §2's certification is the planner's | none | `Claude/Poirot/POIROT-swipe-navstack-settle-window-9506f3a.md` F2 |
| T-NS-F1 | The build log's *corrected* `NAVAPPLIES-a` row declares "14 further" and then names **13** — dropping `NAVAPPLIES (abort token)`, the very cell the correction was about. Poirot's own re-measure is **15** total, superseding its earlier 14 (the 15th was inert when first measured) | the builder | OPEN 2026-08-07, Minor | make the declared count and the named list agree | none | `Claude/Poirot/POIROT-swipe-navstack-settle-window-9506f3a.md` F1 |
| T-NS-F3 | Lifting the last skip left `const SKIP` dead in `test/swipe-navstack-settle.test.js` and a "STATE OF THE SKIPS" header describing a state that commit ended. `test/**` is outside eslint, so nothing catches it | the builder | OPEN 2026-08-07, Minor | delete the dead constant and correct the header | none | `Claude/Poirot/POIROT-swipe-navstack-settle-window-9506f3a.md` F3 |
| T-S7N | ⛔ THE STRIKE'S ORACLE WAS WRONG: "no `@reveal` report ⇒ it threw" proves nothing, because that line is emitted when the observation window CLOSES. The planner's first probe used it and its own control failed | the test author | OPEN 2026-08-06 | do not inherit this oracle; every settle-path cell needs an oracle proven able to FAIL. Carry into stage 7 step 3 as well as the navstack slice | none | `Claude/Plans/PLAN-swipe-navstack-settle-window.md`; `Claude/Loki/STRIKE-swipe-stage7-lease-exits-2026-08-06.md` |
| T-NS-C1 | Coverage-Model defect the test author found by EXECUTING: `NAVRECONCILE`'s §9 fixture, driven literally, is **vacuously green at HEAD on every clause** — the forward sequence throws before the reconcile, and the HEAD landed screen already equals the post-fix value. The suite uses the non-throwing interference dimension 8 already sanctions instead. ⭐A first redesign using `openSub` as interferer was **unsatisfiable by any correct build** (its deferred `overlayFilmstrip` reconcile re-zeroes the panel it opened) | the planner | OPEN 2026-08-06, Note-level, NOT blocking the build | fold the corrected fixture into §9 so the plan and the shipped suite agree | none | `Claude/Curie/RED-swipe-navstack-settle-window.md` C1 |
| T-TOOL1 | A plan's `vitruvius-coverage` block can declare a mutant that does not exist in the registry, and nothing checks it | the planner | OPEN, owed since 2026-08-06 — carried as prose with no row until now | build the check: every declared mutant resolves in `tools/mutate.mjs` | none | `Claude/Zelda/HANDOFF.md` §4 |
| T-TOOL2 | A registration's STATED killers are never compared against its MEASURED ones — the absence of this is what let a registration sit reporting `caught` on cells that no longer existed | the planner | OPEN, owed since 2026-08-06 — carried as prose with no row until now | build the comparison; a stated-but-unmeasured killer is the failure it catches | none | `Claude/Zelda/HANDOFF.md` §4 |
| T-TOOL3 | ⭐⭐⭐A plan's blast-radius equality rule fires on CORRECT work, because the declared figure and the gate's check are taken against different inputs. **STRUCK FOUR TIMES, two sub-causes, and the fourth got past all three review rounds into the BUILD.** ✅ The fourth occurrence is now ABSORBED into the plan (`a630d40`): §8 re-derived over the full mandated set (control 0 → code edits alone 1 → code edits + both comment scrubs **2**), its declared figure moved 1 → 2, and §13 step 5b now names the **change-set** axis beside the tree-state axis. That fixes THIS plan; the tooling below is still owed for every future one. (a) measured from a SUBSET of what the plan itself mandates — stage 7 r1 F1 (a narrower transform than §5 declared); navstack r1 F1 (1 of 3 edits; 3 refused not 1); **and at build time, §8's probe transformed only the 3 CODE edits and never the mandatory COMMENT SCRUBS the same commit requires — 2 refused, not the declared 1, the extra being `EMPTYAFTERHOME-a`.** (b) measured against a different TREE STATE than the gate evaluates — navstack r2 F10 | the planner | OPEN 2026-08-06, widened on the fourth occurrence | build the check: a declared blast radius names BOTH the full change set (code AND comments AND records) and the tree state it was measured against, and the gate evaluates the same two. ⭐The fourth occurrence proves review rounds do not catch this class — only execution does | none | `Claude/Brunel/swipe-navstack-settle-window-build.md`; `Claude/Charpy/PLAN-swipe-navstack-settle-window-2026-08-06-r2.md` F10 |
